...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isfakefullscreen, issticky, ignoretransient;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
static int th = 0;           /* tab bar height */
...
applyrules(Client *c)
{
	...
	if (XGetWindowProperty(dpy, c->win, wmatom[WMWindowRole], 0L, 256L, False, XA_STRING,
		&real, &format, &n, &extra, (unsigned char **)&windowrole) != Success)
		windowrole = NULL;
	role = windowrole ? windowrole : broken;

	for (i = 0; i < LENGTH(rules); i++) {
		r = &rules[i];
		if ((!r->title || fnmatch(r->title, c->name, 0) == 0)
		&& (!r->class || fnmatch(r->class, class, 0) == 0)
		&& (!r->instance || fnmatch(r->instance, instance, 0) == 0)
		&& (!r->role || fnmatch(r->role, role, 0) == 0))
		{
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		if (showsystray && m == systraytomon(m))
			w -= getsystraywidth();
		m->barwin = XCreateWindow(dpy, root, m->wx + sp, m->by, w, bh, 0, DefaultDepth(dpy, screen),
...
togglefakefullscreen(const Arg *arg)
{
    ...
    /* if currently fullscreen, re-apply */
    if (selmon->sel->isfullscreen)
